...
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        ...
        <li><?= $this->Html->link(__('Edit Prioridad'), ['action' => 'edit', $prioridad->id]) ?> </li>
        <li><?= $this->Form->postLink(__('Delete Prioridad'), ['action' => 'delete', $prioridad->id], ['confirm' => __('Are you sure you want to delete # {0}?', $prioridad->id)]) ?> </li>
        <li><?= $this->Html->link(__('List Prioridads'), ['action' => 'index']) ?> </li>
        <li><?= $this->Html->link(__('New Prioridad'), ['action' => 'add']) ?> </li>
        <li><?= $this->Html->link(__('List Tickets'), ['controller' => 'Tickets', 'action' => 'index']) ?> </li>
        <li><?= $this->Html->link(__('New Ticket'), ['controller' => 'Tickets', 'action' => 'add']) ?> </li>
    </ul>
</nav>
<div class="prioridads view large-9 medium-8 columns content">
    <h3><?= h($prioridad->name) ?></h3>
    <table class="vertical-table">
        <tr>
            <th scope="row"><?= __('Name') ?></th>
            <td><?= h($prioridad->name) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Id') ?></th>
            <td><?= $this->Number->format($prioridad->id) ?></td>
        </tr>
    </table>
    <div class="related">
        <h4><?= __('Related Tickets') ?></h4>
        <?php if (!empty($prioridad->tickets)): ?>
        <table cellpadding="0" cellspacing="0">
            ...
            <?php foreach ($prioridad->tickets as $tickets): ?>
            <tr>
                <td><?= h($tickets->id) ?></td>
